...
function BlueHeader({
  ...
  return (
    <div className="bg-blue-500 flex items-center gap-3 px-4 py-4">
      <button onClick={onBack} className="text-black text-2xl font-light hover:opacity-70">
        ←
      </button>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="p-6 mt-6">
        <div className="bg-gray-200 rounded-3xl p-5 space-y-4">
          ...
          {/* Konfirmasi Password */}
          <div>
            <label className="block text-black text-base mb-1">Konformasi Password</label>
            <input
              type="password"
          ...
          admins.map((item, idx) => (
            ...
            >
              ...
              <span className="text-black text-base">
                {item.email}
                {item.super_admin && (
                  <span className="ml-2 text-xs text-blue-600 font-semibold">(Super)</span>
                )}
              </span>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  title={item.super_admin ? "Super admin tidak bisa dihapus" : "Hapus admin"}
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
function KelolaContent() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/auth/admin/${deleteTarget.admin_id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
